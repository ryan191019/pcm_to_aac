...
int  end_wave(int fd, off64_t length)
{				/* only close output */	
	WAVChunkHeader_t cd;	
	off64_t length_seek;	
	off64_t filelen;	u_int rifflen;		
	length_seek = sizeof(WAVHeader_t) +	sizeof(WAVChunkHeader_t) +	sizeof(WAVFmt_t);	
	cd.type = WAV_DATA;	
	cd.length = length > 0x7fffffff ? LE_INT(0x7fffffff) : LE_INT(length);	
	filelen = length + 2*sizeof(WAVChunkHeader_t) + sizeof(WAVFmt_t) + 4;	
	rifflen = filelen > 0x7fffffff ? LE_INT(0x7fffffff) : LE_INT(filelen);	
	if (lseek64(fd, 4, SEEK_SET) == 4)		
		write(fd, &rifflen, 4);
	if (lseek64(fd, length_seek, SEEK_SET) == length_seek)		
		write(fd, &cd, sizeof(WAVChunkHeader_t));
	if (fd != 1)		
		close(fd);
}